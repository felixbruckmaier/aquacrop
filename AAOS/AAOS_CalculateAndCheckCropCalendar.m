...
function [Config, breakloop] = ...
    AAOS_CalculateAndCheckCropCalendar(Config,TestCrop,GDDcumsum,PhenoUnitAOS)
...
% % Set up crop parameters possibly defined by user (except CGC, CDC):
% Pars = struct;
% ParameterNamesGDD = ["Maturity", "Emergence", "Senescence", "HIstart","Flowering"];
% ParameterNamesCD = ["PlantPop","SeedSize", "CCx"];
% ParameterNames = horzcat(ParameterNamesGDD,ParameterNamesCD)';
...
% now it only works if CDC/ CGC are given in GDD:

% Time from sowing to 10% canopy cover (non-stressed conditions)
Vars.Canopy10Pct = round(Pars.Emergence+(log(0.1/Vars.CC0)/Pars.CGC));
...
ConflictNames = ["Emergence","Senescence","Maturity",...
    "HIstart","Flowering"];
for idx_Conflict = 1:size(ConflictNames,2)
    ConflictName = ConflictNames(idx_Conflict);
    ConflictVars.(ConflictName) = Pars.(ConflictName);
end
ConflictVars.MaxCanopy = Vars.MaxCanopy;
ConflictVars.HIend = Vars.HIend;

breakloop = AAOS_CheckPhenologyConstraints(ConflictVars,GDDcumsum);
if breakloop == 0

    Pars.YldFormCD = Vars.HIendCD-Pars.HIstartCD;
    ...
    if Loc_CGC_CDC_InTest >=0
        Unit_CGC_CDC = cellstr(TestCrop.InputUnits(Loc_CGC_CDC_InTest));
    else
        Unit_CGC_CDC = "";
    end
    % If given unit is different than the demanded one, calculate both
    % parameters in the demanded one:

    if Unit_CGC_CDC(1) ~= PhenoUnitAOS

        % Convert CGC
        if Unit_CGC_CDC(1) == "GDD" % if CGC given in GDD...
            CAL_CGC = "_CD";
            CAL = "CD"; % -> CGC_CD will be calculated based on pheno pars in CD.
        elseif Unit_CGC_CDC(1) == "CD" % if CGC given in CD
            CAL_CGC = "";
            CAL = "";
    ...
    if Unit_CGC_CDC(2) ~= PhenoUnitAOS
        ...
        CAL1_CDC = "";
        CAL2_CDC = "";
        if Unit_CGC_CDC(2) == "CD" % if CGC given in CD
            Pars.CDC_CD = Pars.CDC;
            CAL1 = "CD"; % -> CGC will be calculated based on pheno pars in GDD.
            CAL1_CDC = "_CD";
        elseif Unit_CGC_CDC(2) == "GDD" % if CGC given in GDD...
        ...
        if t1 <= 0
            t1 = (CDC_Unit-1) * 4 - 1; % -> fulfill conditions: tCD >= 1; tGDD >= 5
        end
        ...
        if t2 <= 0
            t2 = (CDC_Unit-1) * 4 - 1; % -> fulfill conditions: tCD >= 1; tGDD >= 5
        end
...
%         end
    ...
    NewRow_TestSim = size(Config.TestParameterNames,1)+1;
    NewRow_AllSim = size(AllSimNames,1)+1;
