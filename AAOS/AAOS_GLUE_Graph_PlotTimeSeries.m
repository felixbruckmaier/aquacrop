function [] = AAOS_GLUE_Graph_PlotTimeSeries...
...
Plot2 = plot(-999,0,'-','Color',GraphColors(1,:));
Plot3 = plot(-999,0,'-','Color',GraphColors(7,:));
Plot4 = plot(-999,0,'-','Color',GraphColors(4,:));
Plot6 = plot(-999,0,'pk','MarkerEdgeColor','k','MarkerSize',ms);
Plot7 = plot(-999,0,'pk','MarkerEdgeColor',GraphColors(1,:),'MarkerSize',ms);
Plot8 = plot(-999,0,'pk','MarkerEdgeColor',GraphColors(7,:),'MarkerSize',ms);
Plot9 = plot(-999,0,'pk','MarkerEdgeColor',GraphColors(4,:),'MarkerSize',ms);
...
if any(idx_GoodTest==1); Plot2 = plot(TestVarDays,TestVarSim(idx_GoodTest,:)','-','Color',GraphColors(1,:)); hold on;
    text2 = TestVarNameShort+' ('+TestVarNameShort+'-BS)'; hold on;
    DummyLine2 = xline(-999,'Color',GraphColors(1,:),'linewidth',lw); hold on;
end
% Simulations behavioural towards target variable:
if any(idx_GoodTarget==1); Plot3 = plot(TestVarDays,TestVarSim(idx_GoodTarget,:)','-','Color',GraphColors(7,:)); hold on;
    text3 = TestVarNameShort+' ('+TargetVarNameShort+'-BS)';
    DummyLine3 = xline(-999,'Color',GraphColors(7,:),'linewidth',lw); hold on;
end
% Simulations behavioural towards target AND test variables:
if any(idx_GoodTestAndTarget==1); Plot4 = plot(TestVarDays,TestVarSim(idx_GoodTestAndTarget,:)','-','Color',GraphColors(4,:)); hold on;
    text4 = TestVarNameShort+...
        ' ('+TestVarNameShort+'-&'+TargetVarNameShort+'-BS)';
    DummyLine4 = xline(-999,'Color',GraphColors(4,:),'linewidth',lw); hold on;
end
% Observations:
...
% All Simulations:
HarvestDay = HarvestDays(end);
if any(idx_BadTestAndTarget==1); hold on; Plot6 = plot(HarvestDay+2,TargetVarSim,'pk','MarkerEdgeColor','k','MarkerSize',ms); end
% Simulations behavioural towards test variable:
if any(idx_GoodTest==1); hold on; Plot7 = plot(HarvestDay+3,TargetVarSim(idx_GoodTest)','pk','MarkerEdgeColor',GraphColors(1,:),'MarkerSize',ms);
    text7 = TargetVarNameShort+' ('+TestVarNameShort+'-BS)';
end
% Simulations behavioural towards target variable:
if any(idx_GoodTarget==1); hold on; Plot8 = plot(HarvestDay+4,TargetVarSim(idx_GoodTarget)','pk','MarkerEdgeColor',GraphColors(7,:),'MarkerSize',ms);
    text8 = TargetVarNameShort+' ('+TargetVarNameShort+'-BS)';
end
% Simulations behavioural towards target AND test variable:
if any(idx_GoodTestAndTarget==1); hold on; Plot9 = plot(HarvestDay+5,TargetVarSim(idx_GoodTestAndTarget)','pk','MarkerEdgeColor',GraphColors(4,:),'MarkerSize',ms);
    text9 = TargetVarNameShort+...
        ' ('+TestVarNameShort+'-&'+TargetVarNameShort+'-BS)';
end
% Observations:
Plot10 = plot(HarvestDay+6,TargetVarObs,'pk','MarkerFaceColor','g','MarkerSize',ms);
...
DummyLine2 = xline(-999,'Color',GraphColors(1,:),'linewidth',8);
DummyLine3 = xline(-999,'Color',GraphColors(7,:),'linewidth',8);
DummyLine4 = xline(-999,'Color',GraphColors(4,:),'linewidth',8);
