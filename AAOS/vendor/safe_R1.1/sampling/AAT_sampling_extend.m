function [Xext,r] = AAT_sampling_extend(X,distr_fun,distr_par,Next,extrainput,varargin)
%
...
%% Phenology bounds Check

if any(contains(extrainput.TestParsNames,extrainput.AllPhenoParsConflict_1) == 1)
    X = GSA_phenologybounds(extrainput,X);
    if mod(size(X,1),2) == 1
        X(size(X,1),:) = [];
    end
    r = size(X,1) / 2;
