...
if row_Lot == 0
    ...
    % Calculate biomass/yield NRMSE at harvest day = biomass/yield relative
    % deviation & transform it to GLF which classifies all simulations
    % within the upper 10% range as behavioural (to obtain a GLF that can
    % be compared to the test variable GLFs, which are based on KGE as GoF
    % criteria -> "the more positive, the better" -> requires ">" comparison):
%     GLF_TargetVar = RMSE(TargetVarSim,TargetVarObs) / TargetVarObs;
...
%             KGE_TestVar = nan(SampleLength,1);

cd(Directory.SAFE_util);
%% Calculate the GLF from sim. & obs. values of test variable
% Use NRMSE, normalized to the mean, according to Raes et al. 2018:
GLF_TestVar(:,1) = RMSE(TestVarSim,TestVarObs) / mean(TestVarObs) * 100;


%             for SampleIdx = 1:SampleLength
%                 TestVarSim_i = TestVarSim(SampleIdx,:)';
%                 KGE_TestVar(SampleIdx,1) = ObjFun(TestVarObs,TestVarSim_i);
%             end
%             KGE_TestVar(isnan(KGE_TestVar)) = - sqrt(2);
%             GLF_TestVar = 1 - (1 - KGE_TestVar)/ (1 + sqrt(2));
